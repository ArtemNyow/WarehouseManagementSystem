...
Public Class AnalyticsForm
    ...
    Public Sub New()
        ...
        cmbProductFilter = New ComboBox() ' Ініціалізація до LoadAnalytics
        InitializeLayout()
        LoadAnalytics()
    End Sub
    Private Sub OnModelChanged(sender As Object, e As EventArgs)
        ...
        ' Блокуємо або розблокуємо поля відповідно до моделі
        numAlpha.Enabled = model.Contains("експоненційне")
        numMu.Enabled = model.Contains("M/M/1")
        numS.Enabled = model.Contains("EOQ")
        numH.Enabled = model.Contains("EOQ")

        numIncomeMin.Enabled = model.Contains("Імітаційний")
        numIncomeMax.Enabled = model.Contains("Імітаційний")
        numOutcomeMin.Enabled = model.Contains("Імітаційний")
        numOutcomeMax.Enabled = model.Contains("Імітаційний")
        numStep.Enabled = model.Contains("Імітаційний")

        ' Для "Імітаційного прогнозу запасів" розблоковуємо numDays, а для "Прогнозу (експоненційне згладжування)" блокуємо
        If model.Contains("Імітаційний") Then
            numDays.Enabled = True
        ElseIf model.Contains("експоненційне") Then
            numDays.Enabled = False
        End If

        ' Блокуємо вибір продукту для тих моделей, де він не потрібен
        cmbProductFilter.Enabled = model.Contains("Імітаційний")

        ' Оновлюємо вигляд форми
        LoadAnalytics()
    End Sub
    ...
    Private Sub InitializeLayout()
        ...
        Dim layoutMain As New TableLayoutPanel With {.Dock = DockStyle.Fill, .RowCount = 4, .ColumnCount = 1}
        ...
        panelTop.Controls.AddRange({
            ...
            CreateLabel("α:"), numAlpha,
            CreateLabel("μ:"), numMu,
            CreateLabel("S:"), numS,
            CreateLabel("H:"), numH,
            CreateLabel("Днів:"), numDays,
            CreateLabel("Надходження (від/до):"), numIncomeMin, numIncomeMax,
            CreateLabel("Витрата (від/до):"), numOutcomeMin, numOutcomeMax,
            CreateLabel("Крок (днів):"), numStep
        })

        layoutMain.Controls.Add(panelTop, 0, 0)
        layoutMain.Controls.Add(lblProductInfo, 0, 1)
        layoutMain.Controls.Add(lblSummary, 0, 2)
        layoutMain.Controls.Add(chartDisplay, 0, 3)
    ...
    Private Sub RunQueueingModel(conn As OleDbConnection)
        ...
        Dim rho = lambda / mu
        If rho >= 1 Then
            lblSummary.Text = $"Система нестабільна: λ={Math.Round(lambda, 2)} ≥ μ={mu}. Збільште μ або зменшіть навантаження."
            Return
        End If
        Dim L = rho / (1 - rho)
        Dim W = 1 / (mu - lambda)
        Dim Lq = (rho ^ 2) / (1 - rho)
        Dim Wq = rho / (mu - lambda)
        lblSummary.Text = $"λ={Math.Round(lambda, 2)}, μ={mu}, ρ={Math.Round(rho, 2)} → L={Math.Round(L, 2)}, W={Math.Round(W, 2)}, Lq={Math.Round(Lq, 2)}, Wq={Math.Round(Wq, 2)}"
    End Sub
